      ...
      <form className="form" onSubmit={submit} style={{ position: 'relative', zIndex: 50, pointerEvents: 'auto' }}>
        <div className="admin-reset-actions admin-deadline-control">
          ...
          <div className="admin-reset-select-row admin-deadline-row">
            ...
            <button className="small-btn reset-locked-btn" type="button" onClick={saveDeadline} disabled={savingDeadline || saving || resetting || !deadlineValue}>{savingDeadline ? 'SAVING…' : 'SAVE DEADLINE'}</button>
          </div>
        </div>
